...
exports.createBrand = async (req, res) => {
  try {
    ...
    console.log("-------------brand name-------------");
    const { name } = req.body;
    const result = await BrandPicture.findOneAndUpdate(
      { slug: slugify(name) },
    ...
    if (result) {
      ...
      const newResult = await new BrandPicture({
        name,
        slug: slugify(name),
      }).save();
      console.log("-------------result-------------");
...
exports.getBrandImageInfo = async (req, res) => {
  try {
    const brandInfo = await BrandPicture.findOne({
...
exports.brandList = async (req, res) => {
  const result = await BrandPicture.find({}).sort({ parent: 1 }).exec();
  res.json(result);
};
...
exports.deleteBrandImage = async (req, res) => {
  try {
    ...
    const result = await BrandPicture.findOneAndDelete({slug: slug})
    console.log('result', result)
    if (result){
        const path = `${process.env.URI_BRAND_PICTURE}/${result.fileName}`
...
exports.uploadProductImage = async (req, res) => {
  try {
    ...
    console.log(req.files.image.data);
    console.log("-------------IMAGE PRODUCT-------------");
    const fileName = await `${slug}-${Date.now()}.${
      req.files.image.mimetype.split("/")[1]
    }`;
    await fs.mkdir(
      `${process.env.URI_PRODUCT_PICTURE}/${brand}`,
      async (err, data) => {
        if (err) {
        ...
        fs.mkdir(
          `${process.env.URI_PRODUCT_PICTURE}/${brand}/${slug}`,
          async (err, data) => {
            if (err) {
            ...
            fs.writeFile(
              ...
              async (err, data) => {
                if (err) {
                  ...
                  const result = await ProductPicture.findOneAndUpdate(
                    { name: slug },
                    {
                      $push: {
                        fileName: fileName,
                  ...
                  if (result) {
                    ...
                    const newResult = await new ProductPicture({
                      name: slug,
                      fileName: [fileName],
                      brand,
                    }).save();
